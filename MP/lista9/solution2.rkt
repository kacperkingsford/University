    ...
    [(app f e)
     (let ([vf (eval-env f env)]
       ...
       (match vf [(clo x body fun-env)
                  (eval-env body (env-add x ve (magic-append env fun-env)))]))]))  ;<------------!!!---------------
...
(define (eval-while e env)
  (match e
    ...
    (define init-env (env-add 'x -1 env-empty))

(define (test-env) (env-add 'y 1 (env-add 'x 1 env-empty)))

;--------------!!!--------------------------
; Sprawdza, czy zmienna jest lokalna
(define (is-let x env)
  (define (assoc env how-many)
    (if (null? env)
        (>= how-many 2)
        (if (eq? x (mcar (car env)))
            (assoc (cdr env) (+ how-many 1))
            (assoc (cdr env) how-many))))
  (match env
    [(environ xs) (assoc xs 0)]))


;Procedura pomocnicza (opis)
(define (magic-append global local)
  (match global
    [(environ global-xs)
        (environ (append (make-list global local) global-xs))]))

; Stworzenie listy par ze zmiennymi lokalnymi
(define (make-list global-env local-env)
  (define (assoc g l act)
    (match g
      [(environ g-xs)
          (if (null? g-xs)
              act
              (if (is-let (mcar (car g-xs)) l)
                  (assoc (environ (cdr g-xs))
                         l
                         (cons (mcons (mcar (car g-xs))
                                           (env-lookup (mcar (car g-xs)) l))
                                 act))
                  (assoc (environ (cdr g-xs))
                         l
                         act)))]))
  (assoc global-env local-env null))
