...
(struct apply-expr (proc args) #:transparent) ;<----NOWE
...
(define (expr? e)
  (match e
    ...
    [(apply-expr proc args) (and (lam? proc) (cons-expr? args))]
    [_ false]))


(define (make-list es) ;<----- NOWE
  (foldr cons-expr (null-expr) es))
...
(define (parse q)
  (cond
    ...
    [(and (list? q) (pair? q) (eq? (first q) 'list)) ;<----NOWE
     (make-list (map parse (cdr q)))]
    [(and (list? q) (pair? q) (eq? (first q) 'apply) (eq? (length q) 3))
    (apply-expr (parse (second q)) (parse (third q)))] ;<-----NOWE
...
(define (eval-env e env)
  (match e
    ...
    [(apply-expr proc args) (eval-env (app-desugar proc (to->lst args)) env)]))

(define (eval e) (eval-env e env-empty))

(define (app-desugar proc args)
  (match args
    [(cons-expr e1 e2)
     (match proc
    [(lam x bd)
     (if (cons-expr? e2)
     (match bd
       [(lam y b) (app (app-desugar proc e2) e1)]
       [_ (app proc e1)])
     (app proc e1))]
    [_ (app proc e1)])]))

(define (to->lst xs)
    (mk->lst (reverse (eval xs))))

(define (mk->lst xs)
    (if (null? xs)
        (null-expr)
        (cons-expr (const (car xs)) (mk->lst (cdr xs)))))
